...
//! current force-model fidelity and zero-polar-motion EOP source, not a claim of
//! agency-grade orbit reproduction. The test pins the achieved RMS with `<=`
//! bounds so model improvements pass while regressions fail. On x86-64-linux
//! canonical runs this file pins the listed RMS values directly.

use sidereon_core::astro::forces::SolarRadiationPressure;
use sidereon_core::astro::frames::orientation::EarthOrientationProvider;
use sidereon_core::astro::frames::transforms::PolarMotion;
use sidereon_core::astro::math::least_squares::SolveOptions;
use sidereon_core::astro::propagator::{ForceModelKind, IntegratorKind, IntegratorOptions};
...
fn real_igs_sp3_ecef_fit_converges_and_phase_a_improves_rms() {
    ...
    assert!(
        phase_a_fit.fit_rms_3d_m <= PINNED_PHASE_A_RMS_3D_M,
        "Phase A fit RMS was {:.17e} m",
        phase_a_fit.fit_rms_3d_m
    );
    assert!(
        phase_a_stats.rms_3d_m <= PINNED_PHASE_A_RMS_3D_M,
